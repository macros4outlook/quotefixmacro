...
* We changed the default configuration to be more user convenient:
  ...
  * QuoteFixMacro does not require to redconfigure the signatures any more: `DEFAULT_USE_QUOTING_TEMPLATE` is `True` (instead of `False`)
  * The newly opened window is made as wide as required to display the wrapping is it will be when the email being sent: `DEFAULT_USE_SOFTWRAP` is `True` (instead of `False`)
